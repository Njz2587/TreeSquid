...
public abstract class AIGuardState : AIState
{
    ...
    void Awake()
    {
        ...
        _playerLayerMask = LayerMask.GetMask("Squid", "AI Body Part", "Floor", "Ceiling") + 1;
    ...
    protected virtual bool ColliderIsVisible(Collider other, out RaycastHit hitInfo, int layerMask = -1)
    {
        ...
        Vector3 head = _guardStateMachine.HeadControl.eyeTransform.position;
        ...
        for (int i = 0; i < hits.Length; i++)
        {
            ...
            if (hit.distance < closestColliderDistance)
            {
                ...
                if (hit.transform.gameObject.layer == _bodyPartLayer)
                {
                    ...
                    if (_stateMachine != AIManager.Instance.GetAIStateMachine(hit.rigidbody.GetInstanceID()))
                    {
